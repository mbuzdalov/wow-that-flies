...
class TextMessage(text: String, fontSize: Float, fontColor: Color, x: Float, y: Float,
                  ...
                  timeOn: Double, timeOff: Double) extends GraphicsConsumer:
  ...
  override def consume(img: BufferedImage, g: Graphics2D, time: Double, frameNo: Long): Unit =
    if time >= timeOn && time <= timeOff then
      g.setFont(font)
      val alpha = math.max(0, math.min(1, math.min(time - timeOn, timeOff - time) * 2))
